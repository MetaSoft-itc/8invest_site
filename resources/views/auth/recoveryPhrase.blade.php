...
@section('content')
    ...
    <div class="container">
        <div class="row">
            <div class="col-12">
                <h1 class="section__heading py-4 my-4">{{__("Successfully registered!")}}</h1>
                <p><strong>{{__("Your recovery phrase")}}:</strong></p>
                <div class="alert alert-danger my-4">
                    <p>{{__("Write it down and store securely")}}</p>
                </div>
                <code id="seed-phrase">{{$user->seed_phrase}}
            ...
            <div class="col-12 col-md-4 offset-md-4 my-4">
                <a href="{{route('cabinet')}}" class="btn btn--primary-gr w-100">{{__("Home")}}</a>
            </div>
        </div>
    </div>
    <script>
        ...
        copyBtn.addEventListener('click', ()=>{
            copyToClipboard('{{ $user->seed_phrase }}')
                .then(()=> {
                    animateCopyButton();
                })
